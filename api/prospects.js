export default async function handler(req, res) {
  ...
  const prompt = `You are a lead researcher for Autoflow, an AI receptionist and reputation management SaaS company.
...
- name: the business name or contact person name
- email: a realistic business email (best guess like info@company.com or contact@business.com)
- type: the industry category
- angle: a one-sentence reason why they specifically need Autoflow

Return ONLY a valid JSON array. No markdown, no explanation, no code blocks. Example:
[
  {"name":"River Oaks Dental","email":"info@riveroaksdental.com","type":"Dental","angle":"No weekend answering — losing implant consults to competitors"},
  {"name":"Summit Law Group","email":"contact@summitlaw.com","type":"Law Firm","angle":"Clients call after 5 PM for urgent legal matters, always go to voicemail"}
]
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(200).json({
        prospects: generateFallback(vertical, city, painDesc, requestCount),
    ...
    prospects = prospects.filter(p => p && p.name).map(p => ({
      ...
      angle: String(p.angle || p.why || '').trim()
    }));
...
function generateFallback(vertical, city, pain, count) {
  ...
  const suffixes = {
    ...
    'HVAC and plumbing companies': 'HVAC',
    'auto dealerships': 'Auto',
    'real estate agencies': 'Realty',
    'accounting firms': 'Accounting',
    'insurance agencies': 'Insurance',
    'veterinary clinics': 'Vet',
    'chiropractors': 'Chiropractic',
    'spas and medspas': 'Spa',
    'hotels and motels': 'Inn',
  ...
  const results = [];
  const suffix = suffixes[vertical] || 'Business';
  const type = types[vertical] || 'Business';
